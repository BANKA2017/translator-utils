(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('hpagent')) :
    typeof define === 'function' && define.amd ? define(['hpagent'], factory) :
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.translator = factory(global["{}"]));
})(this, (function (hpagent) { 'use strict';
    ...
    const axiosConfig = async (adds = {}) => {
        ...
        if (typeof process !== 'undefined' && hpagent.HttpProxyAgent && hpagent.HttpsProxyAgent) {
            config.headers = {
                ...config.headers,
            ...
            if (https_proxy) {
                config.httpsAgent = new hpagent.HttpsProxyAgent({ proxy: https_proxy });
            }
            if (http_proxy) {
                config.httpAgent = new hpagent.HttpProxyAgent({ proxy: http_proxy });
            }
        }
        return config;
    };
    ...
    const GoogleBrowserTranslate = async (text = '', target, raw) => {
        ...
        return await new Promise(async (resolve, reject) => {
            axios$1.post('https://translate.googleapis.com/translate_a/t?' + query.toString(), 'q=' + ((text instanceof Array) ? text.map(x => encodeURIComponent(x)).join('&q=') : encodeURIComponent(text)), await axiosConfig()).then((response) => {
    ...
    const GetMicrosoftBrowserTranslatorAuth = async () => {
        try {
            return (await axios$1.get('https://edge.microsoft.com/translate/auth', await axiosConfig())).data;
        }
        catch (e) {
            return '';
        }
    ...
    const MicrosoftBrowserTranslator = async (text = '', target, raw) => {
        ...
        if (jwt) {
            return await new Promise(async (resolve, reject) => {
                axios$1.post(`https://api.cognitive.microsofttranslator.com/translate?from=&to=${target}&api-version=3.0&includeSentenceLength=true`, JSON.stringify(text instanceof Array ? text.map(tmpText => ({ Text: tmpText })) : [{ Text: text }]), await axiosConfig({
                    headers: {
                        'content-type': 'application/json',
    ...
    const SogouBrowserTranslator = async (text = '', target, raw) => {
        ...
        return await new Promise(async (resolve, reject) => {
            axios$1.post('https://go.ie.sogou.com/qbpc/translate', `S-Param=${body}`, await axiosConfig()).then(response => {
