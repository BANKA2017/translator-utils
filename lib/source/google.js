...
const GoogleTranslate = async (text = '', target = 'en', raw) => {
    if (!text) {
        return await Promise.reject('Empty text #GoogleTranslate ');
    }
    if (!SupportedLanguage('google', target)) {
...
const GoogleBrowserTranslate = async (text = '', target = 'en', raw) => {
    if (!text) {
        return await Promise.reject('Empty text #GoogleTranslate ');
    }
    if (!SupportedLanguage('google', target)) {
    ...
    return await new Promise(async (resolve, reject) => {
        axios.post('https://translate.googleapis.com/translate_a/t?' + query.toString(), 'q=' + ((text instanceof Array) ? text.map(x => encodeURIComponent(x)).join('&q=') : encodeURIComponent(text)), await axiosConfig()).then((response) => {
