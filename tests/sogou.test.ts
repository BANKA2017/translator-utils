...
describe('Sogou translator(browser)', () => {
    test.concurrent('English', async ({ expect }) => {
        expect(await SogouBrowserTranslator('hello', 'auto', 'zh-CHS', false)).toMatch('你好')
    })
    test.concurrent('Japanese', async ({ expect }) => {
        expect(await SogouBrowserTranslator('こんにちわ', 'auto', 'zh-CHS', false)).toMatch('你好')
    })
    test.concurrent('Simplified Chinese', async ({ expect }) => {
        expect(await SogouBrowserTranslator('你好', 'auto', 'zh-CHS', false)).toMatch('你好')
    })
    test.concurrent('Korean', async ({ expect }) => {
        expect(await SogouBrowserTranslator('안녕하세요', 'auto', 'zh-CHS', false)).toMatch('你好')
    })
